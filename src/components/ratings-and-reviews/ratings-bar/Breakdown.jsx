...
const Breakdown = (props) => {
  ...
  return (
    <div>
      <p style={{ fontSize: "80%" }}>
        {props.recommend}% of reviews recommend this product
      </p>
      <br />
      <div>
        <p onClick={() => starClick(5)}>
          <u>5 stars</u>:
        </p>
        <BlackBar width={(props.ratings[5] / max) * 100} />
      </div>
      <div>
        <p onClick={() => starClick(4)}>
          <u>4 stars</u>:
        </p>
        <BlackBar width={(props.ratings[4] / max) * 100} />
      </div>
      <div>
        <p onClick={() => starClick(3)}>
          <u>3 stars</u>:
        </p>
        <BlackBar width={(props.ratings[3] / max) * 100} />
      </div>
      <div>
        <p onClick={() => starClick(2)}>
          <u>2 stars</u>:
        </p>
        <BlackBar width={(props.ratings[2] / max) * 100} />
      </div>
      <div>
        <p onClick={() => starClick(1)}>
          <u>1 stars</u>:
        </p>
